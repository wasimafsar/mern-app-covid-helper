...
export default function StickyHeadTable2() {
  ...
  async function getHelps(){
    ...
    const response = await api.get('/canhelp/');
    response.data = response.data.map((rec)=> {
      return {id: rec._id, data : rec.data, mobileNumber: rec.mobileNumber, name:rec.name, type: rec.type == '10'? 'Plasma': 'Oxygen Cylinder'}});
